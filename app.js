...
async function initStreak() {
  ...
  window.streakLogToday = async function (id) {
    ...
    data[idx] = s; await DB.saveStreaks(data); streakRender();
  };
...
async function initLogbook() {
  ...
  async function toggleTask(projId, taskId) { const p = findProj(projId); if (!p) return; const t = findTask(p.children, taskId); if (!t) return; if (t.children && t.children.length) return; t.done = !t.done; t.completedAt = t.done ? now() : null; t._prevStatus = t.done ? 'done' : 'ns'; syncTaskTree(p.children, p); syncProjectState(p); await save(); render(); }
