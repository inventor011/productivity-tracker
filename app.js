...
function toggleLogbookMenu() {
  var sections = document.getElementById('mobile-logbook-sections');
  var arrow = document.getElementById('logbook-submenu-arrow');
  if (!sections) return;
  var isVisible = sections.style.display !== 'none';
  if (isVisible) {
    ...
    if (typeof window.logbookRefreshMobileSections === 'function') window.logbookRefreshMobileSections();
  }
}
(function () {
...
function switchTab(name, persist) {
  ...
  var logbookSections = document.getElementById('mobile-logbook-sections');
  var logbookArrow = document.getElementById('logbook-submenu-arrow');
  if (name !== 'logbook') {
    if (logbookSections) logbookSections.style.display = 'none';
    if (logbookArrow) logbookArrow.classList.remove('expanded');
  } else {
    if (logbookSections) logbookSections.style.display = '';
    if (logbookArrow) logbookArrow.classList.add('expanded');
  }
  if (name === 'ranker' && typeof window.rankerRefreshMenuLabels === 'function') window.rankerRefreshMenuLabels();
  if (name === 'logbook' && typeof window.logbookRefreshMobileSections === 'function') window.logbookRefreshMobileSections();
...
async function initRanker() {
  ...
  function updateRankerMenuLabels() {
    ...
    var promptBtn = document.getElementById('mobile-change-prompt');
    if (apiBtn) apiBtn.innerHTML = '<span class="nav-icon">&#128273;</span> ' + (apiKey ? 'Change API Key' : 'Add API Key');
    if (promptBtn) promptBtn.innerHTML = '<span class="nav-icon">&#128221;</span> ' + (customPrompt ? 'Change Prompt' : 'Add Custom Prompt');
  }
...
async function initLogbook() {
  ...
  function logbookTabLabel(tab) { return tab === 'done' ? 'Completed Projects' : tab === 'log' ? 'Activity Log' : tab === 'archive' ? 'Archived Projects' : 'Active Projects'; }
  function refreshMobileLogbookSections() {
    document.querySelectorAll('[data-logbook-tab]').forEach(function (btn) { btn.classList.toggle('active', btn.dataset.logbookTab === S.tab); });
  }
  window.logbookRefreshMobileSections = refreshMobileLogbookSections;
  window.logbookSwitchSection = function (tab) {
    if (['active', 'done', 'log', 'archive'].indexOf(tab) === -1) return;
    S.tab = tab;
    S._mobileProjectsOpen = true;
    render();
    save();
  };

  function createProject(title) { const p = { id: uid(), title, children: [], createdAt: now(), completedAt: null, archived: false, _lastStatus: 'ns' }; S.projects.push(p); addLog('proj_created', title); S.activeId = p.id; S.tab = 'active'; render(); save(); }
  ...
  function archiveProject(id, archived) { const p = findProj(id); if (!p) return; p.archived = archived; addLog(archived ? 'proj_archived' : 'proj_restored', p.title); S.tab = archived ? 'archive' : (projPct(p) >= 100 ? 'done' : 'active'); S.activeId = p.id; render(); save(); }
  function projectToMarkdown(p) {
    var lines = ['# ' + p.title, '', 'Created: ' + fmtDate(p.createdAt), 'Progress: ' + projPct(p) + '%', 'Status: ' + (p.archived ? 'Archived' : projStatus(p)), ''];
    function walk(t, depth) {
      var mark = t.done ? '[x]' : '[ ]';
      lines.push(Array(depth + 1).join('  ') + '- ' + mark + ' ' + t.title + (t.completedAt ? ' (completed ' + fmtFull(t.completedAt) + ')' : ''));
      if (t.notes) lines.push(Array(depth + 2).join('  ') + 'Notes: ' + t.notes.replace(/\n/g, ' '));
      (t.links || []).forEach(function (l) { lines.push(Array(depth + 2).join('  ') + 'Link: ' + l.label + ' - ' + l.url); });
      (t.children || []).forEach(function (c) { walk(c, depth + 1); });
    }
    (p.children || []).forEach(function (t) { walk(t, 0); });
    return lines.join('\n');
  }
  async function copyProjectData(p) {
    var text = projectToMarkdown(p);
    try { await navigator.clipboard.writeText(text); addLog('proj_copied', p.title); render(); save(); }
    catch (e) { window.prompt('Copy project data:', text); }
  }
  ...
  function logDesc(e) { const b = s => h('b', {}, s); switch (e.action) { case 'proj_created': return [b(e.entity), ' created']; case 'proj_deleted': return [b(e.entity), ' deleted']; case 'proj_done': return [b(e.entity), ' completed']; case 'proj_reopen': return [b(e.entity), ' reopened']; case 'proj_archived': return [b(e.entity), ' archived']; case 'proj_restored': return [b(e.entity), ' restored']; case 'proj_copied': return [b(e.entity), ' copied']; case 'task_created': return ['Task ', b(e.entity), ' added to ', b(e.proj)]; case 'subtask_added': return ['Subtask ', b(e.entity), ' added under ', b(e.extra), ' in ', b(e.proj)]; case 'task_done': return ['Task ', b(e.entity), ' completed in ', b(e.proj)]; case 'task_reopen': return ['Task ', b(e.entity), ' reopened in ', b(e.proj)]; case 'task_deleted': return ['Task ', b(e.entity), ' deleted from ', b(e.proj)]; case 'link_added': return ['Link added to ', b(e.entity), ' in ', b(e.proj)]; default: return [e.action]; } }

  function renderTabs() {
    const el = document.getElementById('lb-tabs');
    el.innerHTML = '';
    ['active', 'done', 'log', 'archive'].forEach(t => {
      const label = t === 'active' ? 'Active' : t === 'done' ? 'Done' : t === 'archive' ? 'Archive' : 'Log';
      el.appendChild(h('div', { className: 'lb-tab' + (S.tab === t ? ' active' : ''), onClick: () => { S.tab = t; render(); } }, label));
    });
  }
  function renderSidebarContent() {
    const el = document.getElementById('lb-sidebar-content');
    el.innerHTML = '';
    function appendProjectRows(list, emptyText) {
      if (!list.length) { el.appendChild(h('div', { className: 'lb-sidebar-empty' }, emptyText)); return; }
      list.forEach(p => {
        const pct = projPct(p);
        const st = projStatus(p);
        const row = h('div', { className: 'lb-proj-row' + (S.activeId === p.id ? ' selected' : ''), onClick: () => { S.activeId = p.id; if (window.innerWidth <= 640) S._mobileProjectsOpen = false; save(); render(); } },
          h('div', { className: 'lb-proj-dot', style: { background: p.archived ? 'var(--lb-faint)' : statusColor(st, pct) } }),
          h('div', { className: 'lb-proj-info' }, h('div', { className: 'lb-proj-title' }, p.title), h('div', { className: 'lb-proj-bar-wrap' }, h('div', { className: 'lb-proj-bar', style: { width: pct + '%', background: statusColor(st, pct) } }))),
          h('div', { className: 'lb-proj-pct' }, pct + '%'));
        el.appendChild(row);
      });
    }
    if (S.tab === 'active') appendProjectRows(S.projects.filter(p => !p.archived && projPct(p) < 100), 'No active projects');
    else if (S.tab === 'done') appendProjectRows(S.projects.filter(p => !p.archived && projPct(p) >= 100), 'No completed projects');
    else if (S.tab === 'archive') appendProjectRows(S.projects.filter(p => p.archived), 'No archived projects');
    else {
      if (!S.log.length) { el.appendChild(h('div', { className: 'lb-sidebar-empty' }, 'No activity yet')); return; }
      S.log.forEach(e => { el.appendChild(h('div', { className: 'lb-log-entry' }, h('div', { className: 'lb-log-dot', style: { background: logColor(e.action) } }), h('div', { className: 'lb-log-body' }, h('div', { className: 'lb-log-desc' }, ...logDesc(e)), h('div', { className: 'lb-log-ts' }, fmtFull(e.ts))))); });
    }
  }
  function renderSidebarFooter() { const el = document.getElementById('lb-sidebar-footer'); el.innerHTML = ''; if (S._addingProject) { const wrap = h('div', { className: 'lb-add-proj-row' }); const inp = h('input', { placeholder: 'Project name…', onKeydown: ev => { if (ev.key === 'Enter' && !ev.shiftKey && inp.value.trim()) { S._addingProject = false; createProject(inp.value.trim()); } if (ev.key === 'Escape') { S._addingProject = false; render(); } } }); const btn = h('button', { onClick: () => { if (inp.value.trim()) { S._addingProject = false; createProject(inp.value.trim()); } } }, 'Add'); wrap.append(inp, btn); el.appendChild(wrap); requestAnimationFrame(() => inp.focus()); } else { el.appendChild(h('button', { className: 'lb-new-proj-btn', onClick: () => { S._addingProject = true; render(); } }, '+ New Project')); } }
  function projectActionsBar(p) {
    const wrap = h('div', { className: 'lb-project-actions' });
    const btn = h('button', { className: 'lb-project-menu-btn', onClick: function (ev) {
      ev.stopPropagation();
      var existing = wrap.querySelector('.lb-dot-menu');
      if (existing) { existing.remove(); return; }
      closeAllDotMenus();
      var menu = h('div', { className: 'lb-dot-menu lb-project-menu' },
        h('button', { className: 'lb-dot-menu-item', onClick: function (e) { e.stopPropagation(); var name = prompt('Rename project:', p.title); if (name && name.trim()) renameProject(p.id, name.trim()); } }, 'Rename Project'),
        h('button', { className: 'lb-dot-menu-item', onClick: function (e) { e.stopPropagation(); archiveProject(p.id, !p.archived); } }, p.archived ? 'Restore Project' : 'Archive Project'),
        h('button', { className: 'lb-dot-menu-item', onClick: function (e) { e.stopPropagation(); copyProjectData(p); } }, 'Copy Project Data'),
        h('button', { className: 'lb-dot-menu-item danger', onClick: function (e) { e.stopPropagation(); if (confirm('Delete project permanently?')) deleteProject(p.id); } }, 'Delete Project')
      );
  ...
  function renderMain() { const el = document.getElementById('lb-main'); el.innerHTML = ''; const p = activeProj(); if (!p) { el.appendChild(h('div', { className: 'lb-empty-state' }, h('div', { className: 'icon' }, '📋'), h('p', {}, 'Select or create a project'))); return; } const pct = projPct(p); const st = projStatus(p); const stLabel = st === 'done' ? 'Completed' : st === 'ip' ? 'In Progress' : 'Not Started'; const header = h('div', { className: 'lb-proj-header' }, h('h1', { style: { cursor: 'pointer' }, title: 'Double-click to rename', onDblclick: function () { var newName = prompt('Rename project:', p.title); if (newName && newName.trim()) renameProject(p.id, newName.trim()); } }, p.title), h('div', { className: 'meta lb-project-summary' }, ...projectSummaryDates(p)), h('div', { className: 'lb-header-bar-wrap' }, h('div', { className: 'lb-header-bar', style: { width: pct + '%', background: statusColor(st, pct) } })), h('div', { className: 'lb-header-row' }, h('div', { className: 'lb-header-pct', style: { color: statusColor(st, pct) } }, pct + '%'), h('div', { className: 'lb-status-badge ' + st, style: st === 'ip' ? { background: pct >= 50 ? 'rgba(39,174,96,.12)' : 'rgba(230,126,34,.12)', color: statusColor(st, pct) } : {} }, stLabel))); el.appendChild(header); el.appendChild(projectActionsBar(p)); if (S.addingTop) { const wrap = h('div', { className: 'lb-add-top-input' }); const inp = h('textarea', { rows: 1, placeholder: 'Task name…', onInput: function(ev) { autoGrowTextarea(ev.target); }, onKeydown: ev => { if (ev.key === 'Enter' && !ev.shiftKey) { ev.preventDefault(); if (inp.value.trim()) { S.addingTop = false; createTask(p.id, inp.value.trim()); } } if (ev.key === 'Escape') { S.addingTop = false; render(); } } }); wrap.appendChild(inp); el.appendChild(wrap); requestAnimationFrame(() => inp.focus()); } else { el.appendChild(h('button', { className: 'lb-add-task-btn', onClick: () => { S.addingTop = true; render(); } }, '+ Add task')); } const tree = h('div', {}); p.children.forEach(t => tree.appendChild(renderTaskNode(t, p))); el.appendChild(tree); }
