  ...
  async function handleSubmit() {
    ...
    if (mode === 'signup') {
      ...
      try {
        ...
        // User already exists (Supabase returns fake success with empty identities)
        if (result.data && result.data.user && result.data.user.identities && result.data.user.identities.length === 0) {
          setError('This email is already registered. Try signing in.');
          return;
        }

        // Session created → logged in immediately
        if (result.data && result.data.session) {
          return; // onAuthStateChange handles the rest
        }

        // No session → email confirmation is still ON in dashboard
        setError('Account created! But email confirmation is enabled in Supabase. Please disable it in Authentication → Providers → Email, then try again.');
